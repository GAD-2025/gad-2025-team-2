...
export function WorkScheduleCalendarStep({
  ...
  return (
    <div className="mx-auto flex min-h-screen w-full max-w-[420px] flex-col bg-white px-4 pb-10 pt-8">
      ...
      <div className="mb-4">
        <h1 className="mb-2 text-xl font-semibold text-gray-900">희망 근무 시작일*</h1>
      ...
      <div className="mb-4 flex items-center justify-between">
        ...
        <span className="text-base font-semibold text-gray-900">{monthYearLabel}</span>
        <button
          type="button"
          onClick={goToNextMonth}
          disabled={!canGoNext}
          className={`px-3 py-1 text-sm ${canGoNext ? 'text-gray-700' : 'text-gray-300'}`}
        >
          →
        </button>
      </div>
      ...
      <div className="mt-auto pt-10">
        <button
          ...
          className={`h-12 w-full rounded-full text-base font-semibold text-white transition ${
            canConfirm ? 'bg-emerald-500' : 'bg-gray-200 text-gray-400'
          }`}
        >
          확인
        </button>
